import { AskRequest, AskResponse, ChatRequest, InterestsResponse, TopicsResponse, ConversationsResponse } from "./models";
...
export async function topicApi(): Promise<TopicsResponse> {
    ...
    const parsedResponse: TopicsResponse = await response.json();
    if (response.status > 299 || !response.ok) {
        throw Error(parsedResponse.error || "Unknown error");
    }
